...
We will start to separate the code that is meant to be a library by putting it in its own [module](https://www.jetbrains.com/help/idea/about-modules.html). Let's suppose we have a project called **client1** and a library called **sdk**
...
You should know there's an option to use [git submodules](https://git-scm.com/docs/git-submodule) (or [svn externals](http://svnbook.red-bean.com/en/1.0/ch07s03.html)) to share modules between projects at the VCS level. This is a valid solution but it is not as comfortable as having multiple projects. I find this solution quite inconvenient especially when you use branching very often.
...
We will declare a [Gradle Dependency](https://docs.gradle.org/current/userguide/artifact_dependencies_tutorial.html) to **sdk** in the same manner as we add any 3rd-party library:
...
[Composite Builds](https://docs.gradle.org/current/userguide/composite_builds.html) is a new feature (added in Gradle since version 3.5) which allows us to override an external dependency with a local one, depending on **OUR** terms. Sweet! Let's see how it works.
